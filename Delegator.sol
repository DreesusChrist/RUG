...
import "./GovernorRUGInterfaces.sol";

contract GovernorRUGDelegate is GovernorRUGDelegateV1, GovernorRUGEvents {

    /// @notice The name of this contract
    string public constant name = "GovernorRUGDelegate";

    /// @notice The minimum setable proposal threshold
    /// @notice change from original contract
    uint public constant MIN_PROPOSAL_THRESHOLDPERCENT = 5000; // 0.50% of sRUG supply : In ten-thosandaths 5000 = 0.50%

    /// @notice The maximum setable proposal threshold
    /// @notice change from original contract
    uint public constant MAX_PROPOSAL_THRESHOLDPERCENT = 10000; // 1.00% of sRUG circulating supply : In ten-thosandaths 10000 = 1.00%
    ...
    /**
      ...
      * @param sRUG_ The address of the sRUG token
      * @param gRUG_ The address of the gRUG token
      * @param votingPeriod_ The initial voting period
      * @param votingDelay_ The initial voting delay
      * @param proposalThreshold_ The initial proposal threshold percent in ten-thousandths
      */
    function initialize(address timelock_, address sRUG_, address gRUG_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_) public {
        require(address(timelock) == address(0), "GovernorRUG::initialize: can only initialize once");
        require(msg.sender == admin, " GovernorRUG::initialize: admin only");
        require(timelock_ != address(0), "GovernorRUG::initialize: invalid timelock address");
        require(sOHM_ != address(0), "GovernorRUG::initialize: invalid sRUG address");
        require(gOHM_ != address(0), "GovernorRUG::initialize: invalid gRUG address");
        require(votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD, "GovernorRUG::initialize: invalid voting period");
        require(votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY, "GovernorRUG::initialize: invalid voting delay");
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(initialProposalId != 0, "GovernorRUG::propose: Governor RUG not active");
        /// @notice change from original contract
